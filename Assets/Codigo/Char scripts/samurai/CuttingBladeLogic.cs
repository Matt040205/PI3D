...
public class CuttingBladeLogic : MonoBehaviour
{
    ...
    private IEnumerator DashCoroutine(GameObject quemUsou)
    {
        ...
        {
            finalPosition = groundHit.point + (Vector3.up * (controller.height / 2f) * 0.9f);
        }
        ...
        float dashRadius = 2f;
        RaycastHit[] hits = Physics.SphereCastAll(startPosition, dashRadius, dashDirection, dashDistance);
